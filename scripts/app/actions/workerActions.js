define(['pixi', 'app/gameContainer', 'cards/cards'], function(PIXI, gameContainer, cards) {
    function takeWorker(card, drawnCard) {        
        ...
        var newCard = cards.worker.create(card.id, "hand", "S");
    ...
    function replaceWorker(card, drawnCard) {
        ...
        var newCard = cards.worker.create(drawnCard.id, "init","S");
    ...
    function resize(card) {
        ...
        close();
        var bigCard = cards.worker.create(card.id, "preview" + card.placement,"B");
        ...
        bigCard.hitArea = new PIXI.Rectangle(0, 0, gameContainer.card.workerCard.x + 225, gameContainer.card.workerCard.y + 300);
